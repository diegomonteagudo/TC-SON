...
class mydsp : public dsp {
 ...
 public:
	...
	void metadata(Meta* m) { 
		...
		m->declare("filename", "pitch_shift2.dsp");
	...
	virtual void buildUserInterface(UI* ui_interface) {
		ui_interface->openVerticalBox("pitch_shift2");
		ui_interface->addHorizontalSlider("notes", &fHslider0, FAUSTFLOAT(2.0f), FAUSTFLOAT(2.0f), FAUSTFLOAT(8.0f), FAUSTFLOAT(1.0f));
		ui_interface->closeBox();
	}
	...
	virtual void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		for (int i0 = 0; i0 < count; i0 = i0 + 1) {
			fRec0[0] = std::fmod(fRec0[1] + 513.0f - fSlow0, 512.0f);
			float fTemp0 = std::min<float>(0.00390625f * fRec0[0], 1.0f);
			float fTemp1 = fRec0[0] + 512.0f;
			...
			output0[i0] = FAUSTFLOAT((fVec0[(IOTA0 - std::min<int>(514, std::max<int>(0, iTemp5))) & 1023] * (fTemp6 + (1.0f - fRec0[0])) + (fRec0[0] - fTemp6) * fVec0[(IOTA0 - std::min<int>(514, std::max<int>(0, iTemp5 + 1))) & 1023]) * fTemp0 + (fVec0[(IOTA0 - std::min<int>(514, std::max<int>(0, iTemp4))) & 1023] * (fTemp2 + (-511.0f - fRec0[0])) + fVec0[(IOTA0 - std::min<int>(514, std::max<int>(0, iTemp4 + 1))) & 1023] * (fRec0[0] + (512.0f - fTemp2))) * (1.0f - fTemp0));
			fRec0[1] = fRec0[0];
			IOTA0 = IOTA0 + 1;
		}
	}
	...
	#define FAUST_FILE_NAME "pitch_shift2.dsp"
	...
	FAUST_ADDHORIZONTALSLIDER("notes", fHslider0, 2.0f, 2.0f, 8.0f, 1.0f);

	#define FAUST_LIST_ACTIVES(p) \
		p(HORIZONTALSLIDER, notes, "notes", fHslider0, 2.0f, 2.0f, 8.0f, 1.0f) \
...
pitch_shift2::pitch_shift2() : AudioStream(FAUST_INPUTS, new audio_block_t*[FAUST_INPUTS])
{
#ifdef NVOICES
    int nvoices = NVOICES;
...
void pitch_shift2::updateImp(void)
{
#if MIDICTRL
...
void pitch_shift2::update(void) { updateImp<FAUST_INPUTS, FAUST_OUTPUTS>(); }

void pitch_shift2::setParamValue(const std::string& path, float value)
{
    fUI->setParamValue(path, value);
}

float pitch_shift2::getParamValue(const std::string& path)
{
